...
curl -s -X POST "$BASE_URL/workflows/$UUID/events/process_payment" \
  -H "Content-Type: application/json" \
  -d '{ "triggerMetadata": { "actor": "jane@example.com", "source": "checkout-ui" } }' | jq .
echo
...
echo "=== 6. Requesting refund (will FAIL) ==="
curl -s -X POST "$BASE_URL/workflows/$UUID/events/request_refund" \
  -H "Content-Type: application/json" \
  -d '{
    "triggerMetadata": { "actor": "jane@example.com", "reason": "Item defective" },
    "subject": { "forceFailure": true }
  }' | jq .
echo

echo "=== Current state (should be refund_failed) ==="
curl -s "$BASE_URL/workflows/$UUID" | jq '.currentState'
echo

echo "=== 7. Retrying refund (will SUCCEED) ==="
curl -s -X POST "$BASE_URL/workflows/$UUID/events/retry_refund" \
  -H "Content-Type: application/json" \
  -d '{ "triggerMetadata": { "actor": "support@company.com", "reason": "Manual retry after gateway fix" } }' | jq .
echo
